...
public class SelectPageMacroSyntaxCheck extends MacroSyntaxCheck implements SyntaxChecker {
    String template = "[selectPage][sql][pageNo=:pageNo,pageSize=:pageSize,rownum=请求行数][查询字段][条件语句][自定义动态条件][分组排序语句]";
    String doc = """
            ...
            例1: @JRESMacro("[selectRowNum][select * from crt_sys_arg][rownum=1000][][user_id = :userId]")
            或者   @JRESMacro("[selectRowNum][select * from crt_sys_arg where user_id = :userId][rownum=1000]")
    ...
    public String performSyntaxCheck(List<String> params) {
        ...
        // [pageNo=:pageNo,pageSize=:pageSize,rownum=请求行数]
        ...
        if (StringUtils.isBlank(param2) || !param2.toLowerCase().contains("rownum")) {
            return generateError("错误：无法解析[pageNo=:pageNo,pageSize=:pageSize,rownum=请求行数].", template, doc);
        }
